...
public function listarUsuarios(){
    ...
    $sql = "SELECT U.IdUsuario, U.NombreCompleto, U.Usuario, U.Correo, U.Estado, U.IdRol, R.rolCommit,
          U.nacimiento, U.habilidades, U.mensaje
          FROM usuarios AS U
...
public function getDatosUsuarioById(){
  $sql = "SELECT *
          FROM usuarios WHERE IdUsuario = {$this->idUsuario};";
  $data = $this->conexion->getTable($sql);
  $row = mysqli_fetch_assoc($data);
  return $row;
}
...
{
  $sql = "UPDATE usuarios SET memberCode = '{$this->memberCode}'
  WHERE IdUsuario = '{$this->idUsuario}'";
  $res = $this->conexion->setQuery($sql);
  return $res;
}
