...
public function datosUsuarioId($id)
{
  ...
  $res = $this->modeloUsuario->getDatosUsuarioById();
  return $res;
}
...
public function joinUsC($usuario, $nombreC, $correo, $idRol, $idUsuario,
                        $fecha, $habilidades, $mensaje, $pais)
{
  $this->modeloUsuario->__SET("usuario", $usuario);
  $this->modeloUsuario->__SET("nombreCompleto", $nombreC);
  $this->modeloUsuario->__SET("correo", $correo);
  ...
  $this->modeloUsuario->__SET("pais", $pais);
  $res = $this->modeloUsuario->joinUsM();
  return $res;
}
public function asignarCodigo($idU, $code)
{
  $this->modeloUsuario->__SET("idUsuario", $idU);
  $this->modeloUsuario->__SET("memberCode", $code);
  $res = $this->modeloUsuario->newMember();
}
}
?>
